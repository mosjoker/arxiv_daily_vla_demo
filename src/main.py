...
def main(target_date: date):
    ...
    if os.path.exists(json_filepath):
        ...
        # --- 2. 过滤论文、论文打分 --- #
        logging.info("步骤 2: 使用 AI 过滤论文并打分 (主题: VLA, E2E, WorldModel)...")
        # 注意：filter_papers_by_topic 依赖 OPENROUTER_API_KEY 环境变量
        filtered_papers = filter_papers_by_topic(
            raw_papers,
            topic="Vision Language Model, VLM, Vision Language Action Model, VLA, World Model, WM, End-to-End Autonomous Driving",
        )
        filtered_papers = rate_papers(filtered_papers)
        # 将filtered_papers按照overall_priority_score降序排序
        filtered_papers.sort(
            key=lambda x: x.get("overall_priority_score", 0), reverse=True
        )
    ...
    try:
        ...
        try:
            if os.path.exists(DEFAULT_HTML_DIR) and os.path.isdir(DEFAULT_HTML_DIR):
                ...
                logging.warning(
                    f"HTML 目录 '{DEFAULT_HTML_DIR}' 不存在，无法生成 reports.json。"
                )
        ...
        logging.error(
            f"模板文件 '{DEFAULT_TEMPLATE_NAME}' 未在 '{DEFAULT_TEMPLATE_DIR}' 中找到。"
        )
    except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="抓取、过滤并生成 arXiv cs.CV 论文的每日报告。"
    )
    parser.add_argument(
        "--date",
        type=str,
    ...
    if not os.path.exists(DEFAULT_TEMPLATE_DIR) or not os.path.exists(
        os.path.join(DEFAULT_TEMPLATE_DIR, DEFAULT_TEMPLATE_NAME)
    ):
        logging.warning(
            f"模板目录 '{DEFAULT_TEMPLATE_DIR}' 或模板文件 '{DEFAULT_TEMPLATE_NAME}' 不存在。HTML 生成可能会失败。"
        )
        # 可以考虑在这里创建默认模板或退出
